...
example = Node (Node (Leaf 1) (Node (Leaf 2) (Leaf 3)))
...
next :: WithCounter a -> (a -> WithCounter b) -> WithCounter b
next ma famb = WithCounter
  $ \i -> let (x, i') = evalWithCounter ma i in evalWithCounter (famb x) i'
...
relabelWithNext (Node l r) = relabelWithNext l
  `next` \l' -> relabelWithNext r `next` \r' -> pure' $ Node l' r'
...
bind sa sasa =
  State $ \s -> let (x, s') = evalState sa s in evalState (sasa x) s'
...
validatePerson' :: String -> Integer -> Maybe Person
validatePerson' name age =
  validateName name `then_` \name' -> validateAge age `then_` \age' -> Just $ Person name' age'
